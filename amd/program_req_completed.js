...
async function local_gcs_get_students() {
    ...
    if (studseldiv.length == 0) {
        ...
        local_gcs_ajax.queue('local_gcs_students_get_all',[]);
        local_gcs_ajax.queue('local_gcs_programs_get',[]);
		var rsp = await local_gcs_ajax.process();
		local_gcs_programs = rsp[1]['data'];
...
async function local_gcs_get_enrollments(studsel) {
    var studsel = document.getElementById('studentselect');
    ...
    local_gcs_ajax.queue('local_gcs_enrollments_get',{'id':studid});
	var r = await local_gcs_ajax.process();
	local_gcs_enrollments = r[0].data;
...
function local_gcs_get_courses_completed(pgmsel) {
    ...
    if (pgmcode != '') {
        local_gcs_ajax.call('local_gcs_program_req_completed_get',
            {'id':studid, 'programcode':pgmcode},
            local_gcs_got_courses_completed);
    }
}

function local_gcs_got_courses_completed(matrix) {
    var studsel = document.getElementById('studentselect');
    var x = studsel.selectedIndex;
    var studid = parseInt(studsel[x].value);
   ...
   // Student name and address block.
    ...
    var printicondiv = document.getElementsByClassName('print');
	if (!printicondiv.length) {
		var pgmseldiv = document.getElementsByClassName('programselect');
		...
		// We need to reapply the change handler since we re-created the select element.
		var pgmsel = document.getElementById('programselect');
		pgmsel.onchange = function() { local_gcs_get_courses_completed(this); };
	}
}
